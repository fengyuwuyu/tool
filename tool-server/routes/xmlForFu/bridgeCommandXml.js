var bridgeCommandXmlModel = require('./../../models/bridgeCommandXml').bridgeCommandXml;
...
exports.create = function(req, res) {
    ...
    var json = {};
    bridgeCommandXmlModel.create({jsonStr: jsonStr, version: 0, clientVersion: initClientVersion }, function(err, bridgeCommandXml) {
        json.result = !!!err;
        json.bridgeCommandXml = {
            id: bridgeCommandXml._id,
            jsonStr: jsonStr
        }
...
exports.checkVersion = function(req, res) {
    ...
    var id = req.body.id;
    bridgeCommandXmlModel.find({"_id":ObjectID(id)}, function(err, results) {
        if (!results.length) {
            res.send({
                result: false,
                msg: "后台已不存在该条数据"
...
exports.update = function(req, res) {
    ...
    bridgeCommandXmlModel.find({"_id":ObjectID(id)}, function(err, results) {
        ...
        var dirOld = "./export/xmlForFu/bridgeCommandXml/";
        var fileOld = dirOld + "/" + nameOld + ".xml";
        checkEmptyDirAndRemove(fileOld, dirOld);

        bridgeCommandXmlModel.update({"_id": ObjectID(id)}, updateJson, {}, function (err) {
            json.result = !!!err;
            json.msg = !err ? "修改成功" : "修改失败，见后台log:" + JSON.stringify(err);
            json.result && (json.version = updateJson.version);
            res.send(json);
            if (json.result) {
                var file = "./export/xmlForFu/bridgeCommandXml/" + name + ".xml";
...
exports.remove = function(req, res) {
    var id = req.body.id;
    var name = req.body.name;
    bridgeCommandXmlModel.remove({"_id":ObjectID(id)}, function(err) {
        res.send({
            result: !!!err,
...
exports.list = function(req, res) {
    ...
    bridgeCommandXmlModel.find({clientVersion : version}, function(err, results) {
        if(err) {
            res.send({
                result: false,
                msg: err
        ...
        for(var i=0, len=results.length; i<len; i++) {
            var bridgeCommandXml = results[i];
            list.push({
                jsonStr: bridgeCommandXml.jsonStr,
                version: bridgeCommandXml.version,
                id: bridgeCommandXml._id
            });
        }
