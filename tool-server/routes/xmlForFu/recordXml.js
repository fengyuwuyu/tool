...
exports.create = function(req, res) {
    ...
    var json = {};
    recordXmlModel.create({jsonStr: jsonStr, version: 0, clientVersion: initClientVersion  }, function(err, recordXml) {
        json.result = !!!err;
        json.recordXml = {
            id: recordXml._id,
            jsonStr: jsonStr,
...
exports.checkVersion = function(req, res) {
    ...
    recordXmlModel.find({"_id":ObjectID(id)}, function(err, results) {
        console.log(err);
        if (!results.length) {
            res.send({
                result: false,
...
exports.remove = function(req, res) {
    var id = req.body.id;
    var name = req.body.name;
    recordXmlModel.remove({"_id":ObjectID(id)}, function(err) {
        res.send({
            result: !!!err,
...
exports.list = function(req, res) {
    ...
    recordXmlModel.find({clientVersion : version}, function(err, results) {
        if(err) {
            res.send({
                result: false,
                msg: err
        ...
        for(var i=0, len=results.length; i<len; i++) {
            var recordXml = results[i];
            list.push({
                jsonStr: recordXml.jsonStr,
                version: recordXml.version,
                id: recordXml._id
            });
        }
