...
import { HttpCommandXmlService } from './Service';
...
@Component({
    selector: 'vwvo-xmlMakerForFu-httpCommandXml',
    templateUrl: './httpCommandXml.html'
})
export class XmlMakerForFuHttpCommandXmlComponent implements OnInit {
    ...
    private httpCommandXmlList: any = [];
    private httpCommandXmlListBak: any = [];
    ...
    constructor(private sanitizer: DomSanitizer, private httpCommandXmlService: HttpCommandXmlService, private clientComponent: XmlMakerForFuComponent, el: ElementRef, private detectorRef: ChangeDetectorRef) {
    ...
    ngOnInit() {
        this.httpCommandXmlService.list({
            successCallback: (list: any)=> {
                this.httpCommandXmlList = list;
                this.httpCommandXmlListBak = this.httpCommandXmlList;
                this.isBusy = false;
            }
        });
        var me = this;
    ...
    filterLabel() {
        ...
        this.timer = setTimeout(function(){
            ...
            if(str && str.trim() != ''){
                me.httpCommandXmlList = [];
                me.httpCommandXmlListBak.forEach(function(obj: any){
                    if(obj.json.root.module._.name && (obj.json.root.module._.name + '').toLocaleLowerCase().indexOf(str.trim().toLocaleLowerCase()) != -1){
                        me.httpCommandXmlList.push(obj);
                    }
                });
                if(me.httpCommandXmlList.length == 1){
                    me.changeCommandXml(me.httpCommandXmlList[0]);
                }
            }else{
                me.httpCommandXmlList = me.httpCommandXmlListBak;
            }
    ...
    removeCommandXmlById(id: string) {
        ...
        let httpCommandXmlList = this.httpCommandXmlList;
    ...
    changeCommandXml(httpCommandXml: any) {
        ...
        this.httpCommandXmlList.forEach((obj: any)=> {
    ...
    createCommandXml() {
        ...
        this.httpCommandXmlService.create({
            ...
            successCallback: (e: any)=> {
                this.isBusy = false;
                e.httpCommandXml.json = JSON.parse(e.httpCommandXml.jsonStr);
                e.httpCommandXml.version = 0;
                this.httpCommandXmlList.push(e.httpCommandXml);
                this.changeCommandXml(e.httpCommandXml);
            }
        });
    }
        ...
        try {
            ...
            this.httpCommandXmlService.update({
                ...
                successCallback: (e: any)=> {
                    ...
                    this.activeCommandXml.downloadUrl = ServerUrl+"xmlForFu/httpCommandXml/"+name+".xml";
                },
                failCallback: (e: any)=> {
                    this.isBusy = false;
                    alert(e.msg);
    ...
    calcJSON2Code(xml: string) {
        ...
        tmpHtml += calcStructArray2CodeStruct(struts, 'command', false, "httpCommandXml", this.activeXmlId);
        if(tmpHtml) {
            ...
            commands.forEach((obj)=> {
                ...
                items.forEach((iObj, iIndex)=> {
                    if(+iObj._.out == 1) {
                        ...
                        if(dataType.indexOf(iObj._.type) == -1){
                            html += `<span class="type">${iObj._.private?"private"+littleSpace:""}</span><a class="type" target="showModeWin" href="javascript: toShowModel('httpCommandXml', \'${iObj._.type}\', \'${this.activeXmlId}\')">${iObj._.type}</a>${iObj._.attribute == "repeated"?"[]":""}${littleSpace}${iObj._.name};${newLine}`;
                        }else{
                ...
                if(type == "Request") {
                    ...
                    outputItems.forEach((iObj, iIndex)=> {
                        ...
                        if(dataType.indexOf(iObj._.type) == -1){
                            html += `<a class="type" target="showModeWin" href="javascript: toShowModel('httpCommandXml', \'${iObj._.type}\', \'${this.activeXmlId}\')">${iObj._.type}</a>${iObj._.attribute == "repeated"?"[]":""}${littleSpace}${iObj._.name};${newLine}`;
                        }else{
        ...
        if(notifications && notifications.length > 0){
            ...
            notifications.forEach(function(obj, index){
                ...
                items.forEach((iObj, iIndex)=> {
                    ...
                    if(dataType.indexOf(iObj._.type) == -1){
                        html += `<span class="type">${iObj._.private?"private"+littleSpace:""}</span><a class="type" target="showModeWin" href="javascript: toShowModel('httpCommandXml',\'${iObj._.type}\', \'${xmlId}\')">${iObj._.type}</a>${iObj._.attribute == "repeated"?"[]":""}${littleSpace}${iObj._.name};${newLine}`;
                    }else{
                        html += `<span class="type">${iObj._.private?"private"+littleSpace:""}</span><span class="type">${iObj._.type}</span>${iObj._.attribute == "repeated"?"[]":""}${littleSpace}${iObj._.name};${newLine}`;
                    }
                });
