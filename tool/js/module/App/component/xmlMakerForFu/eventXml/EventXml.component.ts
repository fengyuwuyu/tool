...
import { EventXmlService } from './Service';
...
@Component({
    selector: 'vwvo-xmlMakerForFu-eventXml',
    templateUrl: './EventXml.html'
})
export class XmlMakerForFuEventXmlComponent implements OnInit {
    ...
    private xmlName: string = "eventXml";

    constructor(private sanitizer: DomSanitizer, private eventService:EventXmlService, private clientComponent: XmlMakerForFuComponent, el:ElementRef, private detectorRef: ChangeDetectorRef) {
    ...
    saveEventXml() {
        ...
        try {
            ...
            this.eventService.update({
                ...
                successCallback: (e:any)=> {
                    this.isBusy = false;
                    this.activeEventXml.downloadUrl = ServerUrl + "xmlForFu/eventXml/" + name + ".xml";
    ...
    calcJSON2Code(xml:string) {
        ...
        html += calcEnumArray2CodeEnum(enums, 'eventXml');
        html += calcStructArray2CodeStruct(struts, 'command', false,'eventXml',this.xmlId);
        ...
        events.forEach((event, mIndex)=> {
            ...
            items.forEach((iObj, iIndex)=> {
                ...
                if(dataType.indexOf(iObj._.type) == -1){
                    html += `<span class="type">${iObj._.private?"private"+littleSpace:""}</span><a class="type" target="showModeWin" href="javascript: toShowModel('eventXml', \'${iObj._.type}\', \'${this.xmlId}\')">${iObj._.type}</a>${iObj._.attribute == "repeated"?"[]":""}${littleSpace}${iObj._.name};${newLine}`;
                }else{
